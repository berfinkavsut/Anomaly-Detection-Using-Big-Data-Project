...
def multi_process(flow, topic):

    print(f"Entered:{topic}")
    system_flow = flow
    print("who me")
...
if __name__ == "__main__":
    ...
    BaseManager.register('SystemFlow', SystemFlow)
    manager = BaseManager()
    manager.start()
    inst = manager.SystemFlow(data_dim, props, ens_props, config="cloud", fe=True, fe_config=fe_config, user="elastic",
                                 psw="sBnnldRLdLiJmZotTSbo", verbose=False)

    topics = ["beste", "device2", "ege"]

    processes = [Process(target=multi_process, args=(inst, t)) for t in topics]
