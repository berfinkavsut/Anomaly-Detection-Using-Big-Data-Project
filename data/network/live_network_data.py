...
def get_live_network_data(interface, packet_amount):
    ...
    for i in range(total_length):
        ...
        df.loc[i] = [
            time_relative, ip_src, ip_dst, ip_proto, proto_name, cap[i].length, ip_dsfield, ip_flags, ip_version,
            srcport, dstport, proto_len, tcp_seq, tcp_seq_raw, tcp_nxtseq,  tcp_ack, tcp_ack_raw, tcp_flags,
            tcp_flags_res, tcp_flags_ns, tcp_flags_cwr, tcp_flags_ecn, tcp_flags_urg, tcp_flags_ack, tcp_flags_push,
            tcp_flags_reset, tcp_flags_syn,  tcp_flags_fin,  tcp_flags_str, window_size_value, window_size,
